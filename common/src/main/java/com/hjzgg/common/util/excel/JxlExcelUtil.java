package com.hjzgg.common.util.excel;

import com.hjzgg.common.util.date.DateUtil;
import jxl.Cell;
import jxl.Sheet;
import jxl.Workbook;
...
public class JxlExcelUtil {
	 ...
	 * @throws IOException
	 */
	public static List<String[]> getSheetData(InputStream is, int rowIdx) throws Exception {
		Workbook wb = Workbook.getWorkbook(is);
		Sheet sheet = wb.getSheet(0);
		List<String[]> sl = new ArrayList<>();
		if (rowIdx >= sheet.getRows() + 1) {
			return sl;
		}

		for (int i = rowIdx; i < sheet.getRows(); ++i) {
			if (null == sheet.getCell(0, i) || StringUtils.isEmpty(sheet.getCell(0, i).getContents())) {
				continue;
			}
			sl.add(getRowData(sheet.getRow(i)));
		}
		return sl;
	}

	private static String[] getRowData(Cell[] cells) {
		String[] as = new String[cells.length];
		for (int i = 0; i < cells.length; ++i) {
			as[i] = cells[i].getContents();
		}
		return as;
	}
		...
		if (lastIndex >= 0) {
			...
			for (int index = firstIndex; index <= lastIndex; index++) {
				...
				for (int i = 0; i < enFields.length; i++) {
					...
					String fieldValue1 = setSpecialFieldValueByName(enFields[i], objValue, item);
					String fieldValue = fieldValue1 == null ? "" : fieldValue1.toString();
	...
	private static String setSpecialFieldValueByName(String fieldName, Object fieldValue, Object o) throws Exception {
		Field field = getFieldByName(fieldName, o.getClass());
		if (field != null) {
			// 获取字段类型
			Class<?> fieldType = field.getType();
			...
			if (Date.class == fieldType) {
				Date date1 = (Date) fieldValue;
				String date2 = DateUtil.getDateStr(date1, "yyyy-MM-dd HH:mm:ss");
				return date2;
			}
		}
		if (fieldValue == null) {
